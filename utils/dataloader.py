...
def collate_skipgram(batch, text_pipeline): 
    """ # 将数据整理成标准的训练数据的过程，此方法被封装在 dataloader 中，在训练时，load batch data 时，此方法被调用；同时此方法可以内置具体的 text_pipeline， 也就是说此方法是 batch train data 具体的构造方法
    ...
    Long paragraphs will be truncated to contain
    no more that MAX_SEQUENCE_LENGTH tokens.                # 此备注说明此时传入的语料已经可以被切割为 tokens 了
    ...
    for text in batch:
        ...
        for idx in range(len(text_tokens_ids) - SKIPGRAM_N_WORDS * 2):
            ...
            input_ = token_id_sequence.pop(SKIPGRAM_N_WORDS) # 根据上下文窗口，选定 index 作为输入
            outputs = token_id_sequence  # input 确定，那么 output 也就确定
...
def get_dataloader_and_vocab(model_name, ds_name, ds_type, data_dir, batch_size, shuffle, vocab=None):
    ...
    # train data 处理方法2 -  ❗️此写法是错误的。会导致 data loader 出错
    def getvocabwithtoken(x_batch, tokenizer, vocab):
        """
        x_batch: 待被计算的 batch train data
        """
        t1 = tokenizer(x_batch)
        t2 = vocab(t1)
        return t2
    collate_fn_with_extra_param = partial(getvocabwithtoken, tokenizer=tokenizer,vocab=vocab)
    ...
    dataloader = DataLoader(  # collate_fn 参数决定了每一个 batch data 都应该如何处理，因此 text_pipeline 可以自定义，但是重要是还要定义好接受 text_pipeline 的方法，就是collate_fn
        ...
        # 此写法适配上述非匿名函数的写法 - 写法2 - ❗️ 此写法配套上述方法2，这个写法是错误的
        # collate_fn=partial(collate_fn=collate_fn, text_pipeline=collate_fn_with_extra_param)
    )
